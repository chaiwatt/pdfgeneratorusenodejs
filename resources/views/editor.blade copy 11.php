        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('merge-cells').addEventListener('click', function() {
                if (selectedCells.length <= 1) {
                    alert('โปรดเลือกอย่างน้อย 2 เซลล์เพื่อรวม');
                    hideContextMenu();
                    return;
                }
                ...
                // Get min/max row/col indices from selected cells
                let minRow = Infinity, maxRow = -Infinity, minCol = Infinity, maxCol = -Infinity;
                selectedCells.forEach(cell => {
                    const cellCoords = getCellCoordinates(cell);
                    minRow = Math.min(minRow, cellCoords.rowIndex);
                    maxRow = Math.max(maxRow, cellCoords.rowIndex);
                    minCol = Math.min(minCol, cellCoords.colIndex);
                    maxCol = Math.max(maxCol, cellCoords.colIndex);
                });

                const rowSpan = maxRow - minRow + 1;
                const colSpan = maxCol - minCol + 1;

                // Set content of the first cell and remove other selected cells
                let mergedContent = '';
                selectedCells.forEach((cell, index) => {
                    if (index === 0) {
                        // The first cell will be the merged cell
                        cell.setAttribute('rowspan', rowSpan);
                        cell.setAttribute('colspan', colSpan);
                        // Ensure content is preserved, or combine if multiple cells have content
                ...
                // Set the combined content to the first cell (optional, based on desired behavior)
                // If you want to combine content from all merged cells into the first one:
            ...
            document.getElementById('unmerge-cells').addEventListener('click', function() {
                ...
                const cellCoords = getCellCoordinates(cell);
                const startRowIndex = cellCoords.rowIndex;
                const startColIndex = cellCoords.colIndex;
                ...
                for (let r = 0; r < originalRowSpan; r++) {
                    ...
                    if (targetRow) {
                        for (let c = 0; c < originalColSpan; c++) {
                            if (r === 0 && c === 0) {
                                // This is the original cell, do nothing
                                continue;
                            }
                            // Create new cells and insert them at the correct position
                            const newCell = createNewCell(table.classList.contains('table-bordered'));
                            // Find the correct insertion point in the targetRow
                            // This part can be tricky with complex merges, a simpler approach is to append and then sort/rearrange if needed
                            let insertBeforeCell = targetRow.cells[startColIndex + c];
                            if (insertBeforeCell) {
                ...
                // Re-evaluate the table structure, especially if there were complex merges.
                // For simplicity, this basic unmerge will add cells back.
                // A more robust solution might involve rebuilding the affected rows/columns.
                hideContextMenu();
            });
            ...
            exportButton.addEventListener('click', () => {
                ...
                .catch(error => {
                    ...
                    showCustomAlert(errorMessage); // Call custom alert function
                    loadingIndicator.style.display = 'none';
                    exportButton.disabled = false;
                });
            });
